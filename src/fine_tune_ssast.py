...
with mlflow.start_run(run_name=str(args.exp_id)):
    ...
    for fold in range(1,5):
        ...
        if args.bal == 'bal':
            ...
            train_loader = torch.utils.data.DataLoader(
                dataloader.AudioDataset(f'{args.data_files}/{fold}_fold_bal_train_data.json', label_csv=args.label_csv, audio_conf=audio_conf),
                batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=False, drop_last=True)

        val_loader = torch.utils.data.DataLoader(
            dataloader.AudioDataset(f'{args.data_files}/{fold}_fold_bal_valid_data.json', label_csv=args.label_csv, audio_conf=val_audio_conf),
        ...
        plt.savefig(f"{args.exp_dir}/{args.exp_name}/{args.exp_id}/fold_{fold}_loss.png")
        log_artifact(f"{args.exp_dir}/{args.exp_name}/{args.exp_id}/fold_{fold}_loss.png")
        ...
        best_model_path = f"{args.exp_dir}/{args.exp_name}/{args.exp_id}/models/fold_{fold}_best_audio_model.pth"
        ...
        log_metric(f"fold_{fold}_train_acc",train_acc)
        log_metric(f"fold_{fold}_val_acc",val_acc)
        log_metric(f"fold_{fold}_test_acc",eval_acc)